...
LAMBDAARN=$(awslocal lambda list-functions --query "Functions[?FunctionName==\`${LAMBDA_FUNCTION}\`].FunctionArn" --output text --region us-east-1)


awslocal lambda list-functions --query "Functions[?FunctionName==ProductsQueryFunction]" --output text
